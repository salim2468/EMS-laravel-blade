...
<div class="container mx-auto p-4">
  <h1 class="text-2xl font-bold mb-4">Apply WFH</h1>
  <form action="{{ route('admin.wfh.store') }}" method="POST" class="max-w-lg mx-auto my-4 bg-white p-6 rounded shadow space-y-4">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <!-- Leave Type -->
    <div>
      <label for="leave_type_id" class="block text-sm font-medium text-gray-700">User</label>
      <select id="leave_type_id" name="leave_type_id" class="mt-1 block w-full border border-gray-300 rounded p-2">
        <option value="personal" {{ old('leave_type_id') === 'personal' ? 'selected' : '' }}>User A</option>
        <option value="sick" {{ old('leave_type_id') === 'sick' ? 'selected' : '' }}>User B</option>
        <option value="floating" {{ old('leave_type_id') === 'floating' ? 'selected' : '' }}>User C</option>
      </select>
    </div>
    <!-- Day Type -->
    <div>
    ...
    <div>
      ...
      <textarea id="description" name="description" rows="3" class="mt-1 block w-full border border-gray-300 rounded p-2"></textarea>
    </div>
    <div class="flex justify-between">
      <button type="submit" class="bg-primary text-white px-4 py-2 rounded hover:bg-blue-700">Apply</button>
